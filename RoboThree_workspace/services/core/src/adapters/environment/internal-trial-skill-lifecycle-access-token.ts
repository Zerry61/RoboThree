...
export const INTERNAL_TRIAL_SKILL_LIFECYCLE_ACCESS_TOKEN_ENV =
...
const ClaimsSchema = z.object({
  ...
  audience: z.literal("enterprise-skill-lifecycle"),
  ...
  permissions: z.tuple([z.literal("skill.manage")]),
}).strict();
...
export class InternalTrialSkillLifecycleAccessToken {
  ...
  static consume(input: Readonly<{
    ...
    clock: Clock;
    variableName?: string;
  }>): InternalTrialSkillLifecycleAccessToken | undefined {
    const variableName = input.variableName ?? INTERNAL_TRIAL_SKILL_LIFECYCLE_ACCESS_TOKEN_ENV;
    ...
    try {
      ...
      return new InternalTrialSkillLifecycleAccessToken(compactToken, claims.expiresAt, input.clock);
    } catch {
      throw invalid();
    }
  }
...
function invalid(): Error {
  return new Error("internal_trial_skill_lifecycle_token_invalid");
}
