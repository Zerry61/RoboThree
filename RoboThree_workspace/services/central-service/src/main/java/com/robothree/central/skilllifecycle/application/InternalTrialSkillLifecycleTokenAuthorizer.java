...
/** Internal-trial-only HS256 verifier for the separately scoped skill.manage token. */
public final class InternalTrialSkillLifecycleTokenAuthorizer {
    private static final ObjectMapper JSON = new ObjectMapper();
    private static final String AUDIENCE = "enterprise-skill-lifecycle";
    private final byte[] verificationKey;
    private final Clock clock;

    public InternalTrialSkillLifecycleTokenAuthorizer(String keyBase64, Clock clock) {
        try { verificationKey = Base64.getDecoder().decode(keyBase64); }
        catch (RuntimeException exception) { throw new IllegalArgumentException("skill lifecycle key invalid"); }
        if (verificationKey.length < 32 || verificationKey.length > 64) {
            throw new IllegalArgumentException("skill lifecycle key must be 32-64 bytes");
        }
        this.clock = clock;
    }
    ...
    public Principal authorize(String compactToken) {
        if (compactToken == null || compactToken.length() < 32 || compactToken.length() > 8192
            ...
            throw SkillLifecycleException.unauthorized();
        }
        try {
            ...
            if (!MessageDigest.isEqual(supplied, expected)) throw SkillLifecycleException.unauthorized();
            ...
            if (!expectedFields.equals(fieldSet(claims))
                    ...
                    || !"skill.manage".equals(claims.path("permissions").path(0).asText())) {
                throw SkillLifecycleException.unauthorized();
            }
    ...
    private static String text(JsonNode node, String field) {
        ...
        if (value == null || !value.isTextual() || value.textValue().isBlank()
                || value.textValue().length() > 160) throw SkillLifecycleException.unauthorized();
        return value.textValue();
    }
    private static String identity(String value) {
        if (!value.matches("^[A-Za-z0-9._:-]{1,160}$")) throw SkillLifecycleException.unauthorized();
        return value;
    }
    ...
    public record Principal(String enterpriseId, String userId, String deviceId) {
        ...
        public String safeSummary() { return "internal-trial-skill-reviewer"; }
    }
}
