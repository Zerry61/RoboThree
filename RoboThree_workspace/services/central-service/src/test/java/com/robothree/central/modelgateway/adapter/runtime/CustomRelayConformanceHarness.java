...
import com.robothree.central.modelgateway.development.DevelopmentModelCredentialMaterialSource;
import com.robothree.central.modelgateway.development.DevelopmentModelCredentialMaterialSource.CredentialKey;
...
class CustomRelayConformanceHarness {

    private static final ObjectMapper JSON = new ObjectMapper();
    private static final String KEY_ENV = "ROBOTHREE_CGF2B3_CUSTOM_RELAY_KEY";
    ...
    private static final String CANARY_ENV = "ROBOTHREE_CGF2B3_RUN_CANARY";
    private static final String DIAGNOSTIC_MARKER =
            "ROBOTHREE_CGF2B3_DIAGNOSTIC=";
    private static final String CONTEXT_SOURCE_DIGEST = "1".repeat(64);

    @Test
    void validatesTheApprovedCustomRelayWithoutPersistingContent() {
        ...
        String upstreamModelId = required(UPSTREAM_MODEL_ENV);
        String canary = required(CANARY_ENV);
        ModelEndpointBinding binding = CustomRelayBindingSeed.create(
                endpoint,
                protocol,
                modelId,
                upstreamModelId);
        ...
        HarnessPublisher normalPublisher = new HarnessPublisher(canary, false);
        Result normal = backend(requestSource, adapter, normalPublisher)
                .execute(
        ...
        Result cancelled = backend(requestSource, adapter, cancellingPublisher)
                .execute(
                        request(
                                binding,
                                cancellationRequest,
        ...
        String safeResult = "{\"status\":\"PASS\",\"deltaCount\":"
                ...
                + "\",\"endpointDigest\":\""
                + CanonicalJson.sha256(endpoint.normalize().toString())
                + "\",\"durationMillis\":"
                + Duration.between(startedAt, Instant.now()).toMillis()
                + ",\"invalidCredential\":\"failed\","
                + "\"cancel\":\"cancelled\",\"deadline\":\"timed_out\"}";
        System.out.println("ROBOTHREE_CGF2B3_RESULT=" + safeResult);
    }
    ...
    private static String providerRequest(
        ...
        model.put("modelRevision", CustomRelayBindingSeed.MODEL_REVISION);
        model.put(
                "configurationRevision",
                CustomRelayBindingSeed.CONFIGURATION_REVISION);
        model.put(
                "runtimeRegistryGeneration",
                CustomRelayBindingSeed.REGISTRY_GENERATION);
    ...
    private static String required(String name) {
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException(
                    "required conformance resource is missing");
        }
        return value;
    }
    ...
    private static void reportFailure(
        ...
        if (errorCode == null || errorCode.isBlank()) {
            errorCode = "model_gateway.custom_relay_outcome_invalid";
        }
    ...
    private static final class HarnessPublisher
        ...
        private HarnessPublisher(String expected, boolean cancelAfterFirst) {
        ...
        @Override
        public synchronized void publishText(UUID invocationId, String delta) {
