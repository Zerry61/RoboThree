...
import { presentRobotReviewDecision, presentRobotReviewDetail, presentRobotReviewOperationError, presentRobotReviewState, validateRejectionReason } from '../../src/presentation/robot-review-presentation';
...
describe('RSL-1 robot review frontend closure', () => {
    it('covers all robot review states in list presentation', async () => {
        const states = ['pending_review', 'approved', 'rejected', 'withdrawn'];
        installRobotReviewAdapter({ pageItems: states.map((state) => createReview({ state })) });
        const wrapper = mount(RobotsPage);
        await flushAsync();
    ...
    it('keeps robot review presentation exhaustive and free of sensitive fields', () => {
        const labels = ['pending_review', 'approved', 'rejected', 'withdrawn']
...
function mountReviewDetail() {
    return mount(RobotDetailPage, {
        mocks: { $route: { params: { robotId: '00000000-0000-4000-8000-000000000009' } } }
    });
}
function installRobotReviewAdapter(options = {}) {
    const calls = { detailLoads: 0, listLoads: 0, approve: [], reject: [] };
