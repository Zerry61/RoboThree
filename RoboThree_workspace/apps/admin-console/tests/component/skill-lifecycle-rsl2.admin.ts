...
import {
  presentEnterpriseSkillDraft,
  presentSkillLifecycleError,
  presentSkillSubmissionDetail,
  presentSkillSubmissionState,
  validateSkillRejectionReason
} from '../../src/presentation/skill-lifecycle-presentation';
import type {
...
describe('RSL-2 Admin skill lifecycle frontend closure', () => {
  it('renders pending, approved, rejected and withdrawn submission states', async () => {
    const calls = installSkillLifecycleAdapter({
      pageItems: (['pending_review', 'approved', 'rejected', 'withdrawn'] as const).map((state) => createSubmission({ state }))
    });

    const wrapper = mount(SkillsPage as unknown as VueConstructor<VueType>);
    await flushAsync();
    ...
    await wrapper.findAll('button').wrappers.find((button) => button.text() === '刷新')?.trigger('click');
    await flushAsync();
    expect(calls.listLoads).toBe(2);
  });
  ...
  it('validates upload format and size before sending multipart archive', async () => {
    ...
    const calls = installSkillLifecycleAdapter({
      uploadDelay: () => new Promise<void>((resolve) => { releaseUpload = resolve; })
    });
    const wrapper = mount(SkillUploadPage as unknown as VueConstructor<VueType>);
    const input = wrapper.find('input[type="file"]');

    await setSelectedFile(input.element as HTMLInputElement, new File(['x'], 'skill.txt', { type: 'text/plain' }));
    ...
    await setSelectedFile(input.element as HTMLInputElement, large);
    expect(wrapper.text()).toContain('技能包超过 200 MiB');

    await setSelectedFile(input.element as HTMLInputElement, new File(['safe'], 'skill.zip', { type: 'application/zip' }));
    await wrapper.findAll('button').wrappers.find((button) => button.text() === '上传并解析')?.trigger('click');
    await flushAsync();

    expect(calls.upload).toHaveLength(1);
    expect(wrapper.text()).toContain('上传请求发送中');
    await wrapper.findAll('button').wrappers.find((button) => button.text() === '上传中')?.trigger('click');
    expect(calls.upload).toHaveLength(1);
    releaseUpload?.();
    await flushAsync();
    await flushAsync();
  ...
  it('keeps package facts immutable and disables restricted scope until permission objects exist', async () => {
    ...
    expect(wrapper.text()).toContain('权限模块提供真实授权对象');
    const restrictedOption = wrapper.findAll('#skill-usage-scope option').wrappers.find((option) => option.text().includes('受限范围'));
    expect((restrictedOption?.element as HTMLOptionElement | undefined)?.disabled).toBe(true);
  });

  it('keeps skill lifecycle presentation and pages free of forbidden display text', async () => {
    expect((['pending_review', 'approved', 'rejected', 'withdrawn'] as const).map((state) => presentSkillSubmissionState(state).label))
...
function mountSubmissionDetail() {
  return mount(SkillSubmissionDetailPage as unknown as VueConstructor<VueType>, {
    propsData: { submissionId: '00000000-0000-4000-8000-000000000021' }
  });
}

async function setSelectedFile(input: HTMLInputElement, file: File): Promise<void> {
...
function mountDraft() {
  return mount(EnterpriseSkillDraftPage as unknown as VueConstructor<VueType>, {
    propsData: { skillId: 'skill.sales-analysis' }
  });
}

function installSkillLifecycleAdapter(options: Readonly<{
  pageItems?: readonly SkillSubmissionDetail[];
  detailSequence?: readonly SkillSubmissionDetail[];
  draftSequence?: readonly EnterpriseSkillDraft[];
  approveError?: AdminApiError;
  approveDelay?: () => Promise<void>;
  uploadDelay?: () => Promise<void>;
}> = {}) {
  const calls: {
    listLoads: number;
    detailLoads: number;
    approve: ApproveSkillSubmissionCommand[];
    reject: RejectSkillSubmissionCommand[];
    upload: Array<{ command: UploadEnterpriseSkillPackageCommand; archive: File }>;
    save: UpdateEnterpriseSkillDraftMetadataCommand[];
    test: StartEnterpriseSkillDraftTestCommand[];
    publish: PublishEnterpriseSkillDraftCommand[];
  } = { listLoads: 0, detailLoads: 0, approve: [], reject: [], upload: [], save: [], test: [], publish: [] };
  ...
  installAdminAdapter({
    ...
    async uploadEnterpriseSkillPackage(command, archive) {
      ...
      if (options.uploadDelay !== undefined) await options.uploadDelay();
...
function receipt(commandId: string, id: string, state: SkillLifecycleMutationReceipt['state']): SkillLifecycleMutationReceipt {
  return {
    contractVersion: 'skill-lifecycle.v1alpha1',
    commandId,
    correlationId: id,
...
function createSubmission(overrides: Partial<SkillSubmissionDetail> = {}): SkillSubmissionDetail {
  return {
...
function createDraft(overrides: Partial<EnterpriseSkillDraft> = {}): EnterpriseSkillDraft {
  return {
    contractVersion: 'skill-lifecycle.v1alpha1',
    skillId: 'skill.sales-analysis',
    draftRevision: revisionA,
