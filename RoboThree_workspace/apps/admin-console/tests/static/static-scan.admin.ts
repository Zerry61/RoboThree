...
import type { StaticScanOptions, StaticScanResult } from '../../scripts/static-scan.mjs.d';
...
const { hasStaticScanFailure, scanStaticSources } = staticScanRuntime as {
  hasStaticScanFailure(result: StaticScanResult): boolean;
  scanStaticSources(options?: StaticScanOptions): Promise<StaticScanResult>;
};

type BundleFixtureOptions = Readonly<{
  production?: 'valid' | 'missing' | 'empty' | 'cssOnly';
  integration?: 'valid' | 'missing' | 'empty' | 'cssOnly';
}>;

async function createBundleFixture(options: BundleFixtureOptions = {}) {
...
async function createBundleRoot(directory: string, mode: 'valid' | 'missing' | 'empty' | 'cssOnly') {
  if (mode === 'missing') {
    return;
  }
