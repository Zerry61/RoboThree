...
describe('Admin integration loopback server', () => {
  it('serves the integration build and proxies only exact same-origin Admin API methods with strict headers', async () => {
    ...
    const upstreamRequests: Array<Readonly<{ method: string; url: string; body: string }>> = [];
    ...
    if (proxyAddress === null || typeof proxyAddress === 'string') throw new Error('proxy address unavailable');
    ...
    for (const [name, value] of Object.entries(ADMIN_SECURITY_HEADERS)) expect(html.headers[name.toLowerCase()]).toBe(value);
...
async function requestHttp(url: string, method: string, headers: Record<string, string> = {}, body?: string): Promise<Readonly<{ status: number; headers: Record<string, string | string[] | undefined>; body: string }>> {
