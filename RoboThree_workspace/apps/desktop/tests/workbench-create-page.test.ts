...
describe("DFE-2A Workbench create page", () => {
  ...
  it("keeps task actions inside one composer with separate resource and model popovers", async () => {
    ...
    expect(composer.find("button[title]").text()).toContain("提交任务");
  ...
  it("fills the task composer from a prototype-aligned quick action without submitting", async () => {
  ...
  it("consumes a follow-up once, validates candidates and requires explicit resources", async () => {
    ...
    expect(wrapper.find(".workbench-page__composer-meta").text()).toContain("Normal Agent");
    ...
    await wrapper.find(".workbench-page__composer-meta button").trigger("click");
    await flushPromises();
  ...
  it.each([
    ...
    expect(wrapper.find(".workbench-page__composer-meta").text()).toContain(
      catalog.agents === undefined ? "Normal Agent" : "请选择机器人",
    );
  ...
  it("renders real catalog projections and submits a task through the adapter", async () => {
    ...
    expect(wrapper.text()).toContain("通用机器人 · 0 个工具");
  ...
  it("submits a normal conversation from the default working directory", async () => {
    ...
    expect(wrapper.text()).toContain("RoboThree 默认工作区");
  ...
  it("keeps a selected robot when refresh marks it unavailable and does not switch to another runnable robot", async () => {
    ...
    expect(wrapper.find(".workbench-page__composer-meta").text()).toContain("Broken Agent");
  ...
  it("keeps robot selection empty across repeated refreshes after the selected robot disappears", async () => {
    ...
    expect(wrapper.find(".workbench-page__composer-meta").text()).toContain("请选择机器人");
    expect(wrapper.find("button[aria-controls='workbench-model-menu']").text())
      .toContain("选择模型");
...
function createConversationTasksAdapter(
  ...
  return { adapter, emit: (event: Parameters<TasksAdapter["subscribe"]>[0] extends
