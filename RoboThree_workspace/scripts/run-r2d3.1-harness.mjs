...
const evidenceDirectory = join(workspaceRoot, "qa-reports", "r2d3.1-runs", runId);
...
try {
  ...
  if (execution.status !== 0) throw typed("r2d31_focused_tests_failed");
  ...
  if (testFileCount !== focusedFiles.length) {
    throw typed("r2d31_focused_file_count_mismatch");
  }
  ...
  for (const [specifier, exportedName, expected, errorCode] of [
    ...
    const imported = spawnSync(process.execPath, [
      ...
      `const module = await import(${JSON.stringify(specifier)});`
        + `if (module[${JSON.stringify(exportedName)}] !== ${JSON.stringify(expected)}) process.exit(2);`,
    ], {
      cwd: join(workspaceRoot, "services/core"),
      encoding: "utf8",
    });
    if (imported.status !== 0) throw typed(errorCode);
  }
  ...
  if (targetSchemaVersion !== 26) throw typed("r2d31_migration_boundary_drift");

  const lockfileDigest = `sha256:${createHash("sha256").update(
    await readFile(join(workspaceRoot, "pnpm-lock.yaml")),
  ).digest("hex")}`;
  if (lockfileDigest !== expectedLockfileDigest) throw typed("r2d31_lockfile_drift");

  const semanticEvidence = Object.freeze({
    outcome: "R2D31_ENTITLEMENT_DECISION_CONTRACT_CONFORMANT",
    runtimeSelectionV1Alpha3PrivateSubpathImportable: true,
    coordinationV1Alpha4PrivateSubpathImportable: true,
    legacyContractDriftCount: 0,
    privateMaterialLeakCount: 0,
    productionR2dConsumerCount: 0,
    ...
    builtInGeneralAgentReady: false,
  ...
  const failure = Object.freeze({
    status: "FAIL",
    outcome: "R2D31_HARNESS_FAILED",
    errorCode: typeof error?.code === "string"
      ? error.code
      : "r2d31_unexpected_failure",
  });
...
function exactCount(output, pattern, label) {
  ...
  if (match?.[1] === undefined) throw typed(`r2d31_${label}_summary_missing`);
